...
def search_phone_number(csv_path, search_value):
    ...
    search_result = search_result.sort_values(by='Phone Number', ascending=True)
...
def search_ip(csv_path, search_value):
    df = pd.read_csv(csv_path, encoding='ANSI')
    columns_to_exclude = ['v_port', 'MAC addr', 'mode', 'attached UC phone', 'Detail Type']
    search_result = df[df['IP address'].astype(str).str.startswith(str(search_value))].drop(columns=columns_to_exclude)
    # IP 주소를 숫자로 변환하여 정렬
    search_result['IP address'] = search_result['IP address'].apply(lambda x: sum([256**i*int(j) for i, j in enumerate(x.split('.')[::-1])]))
    search_result = search_result.sort_values(by='IP address', ascending=True)
    # 다시 IP 주소 형식으로 변환
    search_result['IP address'] = search_result['IP address'].apply(lambda x: '.'.join([str((x >> i) % 256) for i in (24, 16, 8, 0)]))
    return search_result
